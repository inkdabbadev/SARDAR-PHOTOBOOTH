...
module.exports = async function handler(req, res) {
  ...
  const endpoint = String(firstValue(req.query.endpoint) || '').replace(/^\/+/, '');
  if (!ALLOWED_ENDPOINTS.has(endpoint)) {
    res.status(404).json({
      ok: false,
      error: 'Unknown booth endpoint.',
      endpoint,
    });
    return;
  }
  ...
  incomingUrl.searchParams.delete('endpoint');
  const targetUrl = `${boothBase}/${endpoint}${incomingUrl.search}`;
